      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div variants={stagger} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="grid lg:grid-cols-[1fr_1.15fr] gap-12 lg:gap-20 items-start">
          ...
          <motion.div variants={stagger} className="flex flex-col gap-2.5">
            {pts.map((t, i) => (
              <motion.div key={i} variants={fadeUp} className="flex items-start gap-3 px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/25 border border-slate-200/80 dark:border-slate-700/25 hover:border-indigo-500/30 dark:hover:border-indigo-500/20 transition-all group cursor-default">
...
function ExpCard({ e, i }: { e: Experience; i: number; }) {
  const ref = useRef<HTMLDivElement>(null);
...
function Contact({ year }: { year: number; }) {
  const ref = useRef<HTMLDivElement>(null);
  const inView = useInView(ref, { once: true, margin: '-8% 0px' });
  return (
